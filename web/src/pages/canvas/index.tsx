...
export default function CanvasPage() {
    ...
    const createAndEnter = () => enterProject(createProject(`圣诞画布 ${projects.length + 1}`));
    ...
    useEffect(() => {
        ...
        enterProject(mode === "new" ? createProject(`圣诞画布 ${projects.length + 1}`) : projects[0]?.id || createProject(`圣诞画布 ${projects.length + 1}`));
            ...
            <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-10">
                <header className="flex flex-wrap items-end justify-between gap-4 border-b border-stone-200 pb-6 dark:border-stone-800">
                    <div>
                        ...
                        <h1 className="mt-3 text-3xl font-semibold">圣诞画布</h1>
                    </div>
                    <div className="flex items-center gap-2">
                        {selectedIds.length ? (
                            <>
                                <Button disabled={!hydrated} icon={<Download className="size-4" />} onClick={() => void exportCanvasProjects(projects.filter((project) => selectedIds.includes(project.id)), `圣诞画布-${selectedIds.length}个项目`)}>
                                    导出选中
                                </Button>
