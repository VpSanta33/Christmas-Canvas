...
func (s *Store) Create(ctx context.Context, email, passwordHash, displayName, role string, emailVerified bool) (User, error) {
	...
	// Serialize bootstrap registration so concurrent first requests cannot both become admin.
...
func (s *Store) FindByID(ctx context.Context, id string) (User, error) {
	...
	err := s.pool.QueryRow(ctx,
		...
		&u.SessionVersion, &u.EmailVerified)
	if errors.Is(err, pgx.ErrNoRows) {
		return User{}, ErrNotFound
	}
	return u, err
...
func (s *Store) ListUsers(ctx context.Context) ([]AdminUser, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT u.id, u.email, u.email_verified_at IS NOT NULL, u.display_name, u.role, u.disabled,
		        u.created_at
		 FROM users u
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &u.Email, &u.EmailVerified, &u.DisplayName, &u.Role, &u.Disabled, &createdAt); err != nil {
			return nil, err
		}
...
// DeleteUser 删除用户；关联的画布/资产/文件和业务记录由外键 ON DELETE CASCADE
